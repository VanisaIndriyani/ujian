<div class="grid md:grid-cols-2 gap-5">
    <div>
        ...
        <select name="subject_id" class="mt-1 w-full rounded-xl border border-emerald-200 px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-transparent" required>
            ...
            @foreach ($subjects as $subject)
                <option value="{{ $subject->id }}" {{ (int) old('subject_id', $exam->subject_id ?? 0) === $subject->id ? 'selected' : '' }}>
                    {{ $subject->name }}
                </option>
            @endforeach
        </select>
    ...
    <div class="md:col-span-2">
        ...
        <textarea name="description" rows="4" class="mt-1 w-full rounded-xl border border-emerald-200 px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-transparent" placeholder="Ringkasan ujian matakuliah">{{ old('description', $exam->description ?? '') }}</textarea>
    </div>
    ...
    <div class="md:col-span-2">
        ...
        @if(!empty($exam?->material_path))
            <p class="mt-1 text-xs text-emerald-400">Materi saat ini: <span class="text-emerald-600">{{ basename($exam->material_path) }}</span></p>
        @endif
