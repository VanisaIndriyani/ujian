...
@section('content')
    ...
    <div class="bg-white border border-emerald-100 rounded-2xl shadow overflow-hidden">
        <div class="px-4 py-3 border-b border-emerald-100">
            ...
            <p class="text-xs text-emerald-500">Bobot default: Absen 10%, Praktikum 10%, Tugas 20%, UTS 30%, UAS 30%</p>
        </div>

        <form method="POST" action="{{ route('admin.grades.student.save', ['student' => $student->id]) }}" class="p-4 space-y-4 text-sm">
            @csrf
    ...
    <div class="mt-4">
        <a href="{{ route('admin.grades.index', ['subject_id' => $subjectId]) }}" class="text-emerald-600 hover:underline">&larr; Kembali ke halaman Nilai</a>
    </div>
@endsection