...
@section('content')
    <div class="mb-4 bg-white border border-emerald-100 rounded-2xl shadow overflow-hidden">
        <div class="px-4 py-3">
            <form method="GET" action="{{ route('admin.grades.index') }}" class="flex flex-wrap items-center gap-3 text-sm">
                ...
                <a href="{{ route('admin.grades.summary', ['q' => ($q ?? request('q'))]) }}" class="inline-flex items-center gap-2 px-3 py-2 rounded-lg bg-emerald-50 text-emerald-700 hover:bg-emerald-100" aria-label="Ringkasan">
    ...
    <div class="mt-6 bg-white border border-emerald-100 rounded-2xl shadow overflow-hidden">
        <div class="px-4 py-3 border-b border-emerald-100">
            ...
            <p class="text-xs text-emerald-500">Klik ikon untuk input nilai atau export PDF</p>
        </div>
                        ...
                        <tr class="hover:bg-emerald-50/40">
                            ...
                            <td class="px-4 py-3">
                                <div class="flex items-center gap-2">
                                    <a href="{{ route('admin.grades.student.show', ['student' => $stu->id, 'subject_id' => ($subjectId ?? request('subject_id'))]) }}" class="inline-flex items-center justify-center w-9 h-9 rounded-xl border border-emerald-200 text-emerald-600 bg-white hover:bg-emerald-50" title="Input Nilai" aria-label="Input Nilai">
                                        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" class="w-5 h-5">
                                            <path d="M12 5c-7.633 0-11 7-11 7s3.367 7 11 7 11-7 11-7-3.367-7-11-7Zm0 12a5 5 0 1 1 0-10 5 5 0 0 1 0 10Z"/>
                                        </svg>
                                    </a>
                                    <a href="{{ route('admin.grades.student.export', ['student' => $stu->id, 'subject_id' => ($subjectId ?? request('subject_id')), 'w_absen' => 10, 'w_praktikum' => ($weights['w_praktikum'] ?? 10), 'w_tugas' => ($weights['w_tugas'] ?? 20), 'w_uts' => ($weights['w_uts'] ?? 30), 'w_uas' => ($weights['w_uas'] ?? 30)]) }}" class="inline-flex items-center justify-center w-9 h-9 rounded-xl border border-emerald-200 text-emerald-600 bg-white hover:bg-emerald-50" title="Export PDF" aria-label="Export PDF">
                                        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" class="w-5 h-5">
