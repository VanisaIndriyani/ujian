...
<a href="{{ route('guru.subjects.index') }}" class="inline-flex items-center gap-2 px-4 py-2 rounded-lg border border-emerald-200 text-emerald-600 hover:bg-emerald-50">
        ...
        <form action="{{ route('guru.subjects.store') }}" method="POST" class="p-6 space-y-4">
            @csrf
            <div>
<label class="block text-sm font-medium text-emerald-700">Kode</label>
<input type="text" name="code" value="{{ old('code') }}" class="mt-1 w-full rounded-lg border-emerald-200 focus:border-emerald-400 focus:ring-emerald-200" required>
                @error('code')<p class="text-sm text-red-600 mt-1">{{ $message }}</p>@enderror
            </div>
            <div>
<label class="block text-sm font-medium text-emerald-700">Nama</label>
<input type="text" name="name" value="{{ old('name') }}" class="mt-1 w-full rounded-lg border-emerald-200 focus:border-emerald-400 focus:ring-emerald-200" required>
                @error('name')<p class="text-sm text-red-600 mt-1">{{ $message }}</p>@enderror
            </div>
            <div>
<label class="block text-sm font-medium text-emerald-700">Deskripsi</label>
<textarea name="description" rows="4" class="mt-1 w-full rounded-lg border-emerald-200 focus:border-emerald-400 focus:ring-emerald-200">{{ old('description') }}</textarea>
