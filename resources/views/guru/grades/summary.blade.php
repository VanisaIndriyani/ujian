...
@section('content')
    <div class="mb-4 bg-white border border-emerald-100 rounded-2xl shadow overflow-hidden">
        <div class="px-4 py-3">
            <form method="GET" action="{{ route('guru.grades.summary') }}" class="flex flex-wrap items-center gap-3 text-sm">
                ...
                <a href="{{ route('guru.grades.index', request()->query()) }}" class="px-3 py-2 rounded-lg bg-emerald-50 text-emerald-700 hover:bg-emerald-100">Kembali</a>

                <span class="ml-auto"></span>
                <a href="{{ route('guru.grades.export.pdf', request()->query()) }}" class="inline-flex items-center gap-2 px-3 py-2 rounded-lg bg-emerald-600 text-white hover:bg-emerald-700" title="Export PDF" aria-label="Export PDF">
                    <i class="fa-solid fa-file-pdf"></i>
                    <span>Export PDF</span>
                </a>
                <a href="{{ route('guru.grades.export.excel', request()->query()) }}" class="inline-flex items-center gap-2 px-3 py-2 rounded-lg bg-emerald-100 text-emerald-700 hover:bg-emerald-200" title="Export Excel" aria-label="Export Excel">
