...
@section('header', 'Nilai Praktikum')
...
<div class="bg-white border border-emerald-100 rounded-2xl shadow overflow-hidden">
    <div class="px-4 py-3 border-b border-emerald-100">
        <form method="GET" action="{{ route('guru.grades.praktikum') }}" class="flex flex-wrap items-center gap-3 text-sm">
        ...
        <table class="min-w-full divide-y divide-emerald-100 text-sm">
            <thead class="bg-emerald-50/60 text-emerald-600 uppercase text-xs tracking-wider">
                <tr>
                    ...
                    <th class="px-4 py-3 text-left">Judul Praktikum</th>
            ...
            <tbody class="divide-y divide-emerald-50">
                @forelse ($submissions as $sub)
                    <tr class="hover:bg-emerald-50/40">
                        <td class="px-4 py-3 text-emerald-600">{{ $sub->student?->name ?? '—' }}</td>
                        <td class="px-4 py-3 text-emerald-600">{{ $sub->student?->classroom ?? '—' }}</td>
                        <td class="px-4 py-3 text-emerald-600">{{ $sub->assignment?->subject?->name ?? '—' }}</td>
                        <td class="px-4 py-3 text-emerald-600">{{ $sub->assignment?->title ?? '—' }}</td>
                        <td class="px-4 py-3">
                            <form method="POST" action="{{ route('guru.tasks.submissions.update', [$sub->assignment, $sub]) }}" class="flex items-center gap-2">
                                @csrf
                                @method('PATCH')
                                <input type="number" name="score" min="0" max="100" step="0.01" value="{{ old('score', $sub->score) }}" class="w-24 rounded-lg border border-emerald-200 px-2 py-1">
                                <input type="text" name="feedback" placeholder="Catatan" value="{{ old('feedback', $sub->feedback) }}" class="rounded-lg border border-emerald-200 px-2 py-1">
                ...
                @empty
                    <tr>
                        <td colspan="5" class="px-4 py-6 text-center text-emerald-400">Belum ada submissions praktikum.</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
